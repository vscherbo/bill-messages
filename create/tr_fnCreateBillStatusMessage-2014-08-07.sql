...
IF NEW.предок = NEW."№ счета" THEN
   mstr := 'Заказ ' || to_char(NEW."№ счета", 'FM9999-9999');
ELSE 
   mstr := 'Заказ ' || to_char(NEW.предок, 'FM9999-9999') || '/' || to_char(NEW."№ счета", 'FM9999-9999');
END IF;
...
--           RAISE NOTICE 'WHEN % mstr=%', NEW.Статус, mstr  ; 
    WHEN 2 THEN
           mstr := mstr || ' полностью оплачен.' ||  E'\r\nО готовности заказа мы Вас известим дополнительно.' ;
...
--           RAISE NOTICE 'WHEN % mstr=%', NEW.Статус, mstr  ; 
    WHEN 10 THEN 
           ...
           IF delivery = 2 THEN -- отправлен через ТК/Почта/Курьерская служба
              ...
              END IF; -- ДокТКДата
              -- для Деловых линий и Автотрединга добвыляем ссылку (URL)
