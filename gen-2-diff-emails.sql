...
--, regexp_replace(lower(trim(email[1])), '(.*?)([a-z0-9\-\.]+)@([a-z0-9\-\.]+)', '\2@\3', 'g') AS em1 
--, regexp_replace(lower(trim(email[1])), '.*?([\w\-+]+(?:\.[\w\-+]+)*@(?:[\w\-]+\.)+[a-zA-Z]{2,7})', '\1', 'gi') AS em1
,regexp_replace(trim(email[1]), '([\w\-+]+(?:\.[\w\-+]+)*@(?:[\w\-]+\.)+[a-zA-Z]{2,7}) ', '\1', 'gi') AS em1 
